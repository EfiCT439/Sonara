...
- `routes/ai.js` at `/api/ai` — **MusicGen (instrumental):** `POST /api/ai/song` (Replicate) → `{ predictionId }`, `GET /api/ai/song/status/:id` → `{ status, audioUrl }` (needs `REPLICATE_API_TOKEN`). **Suno (full songs WITH vocals):** `POST /api/ai/suno` → `{ taskId }`, `GET /api/ai/suno/status/:taskId` → `{ status:'succeeded', audioUrl, title, imageUrl, songs[] }`. Suno has no official API yet, so this hits a **third-party provider** (`SUNO_API_BASE`, default sunoapi.org) with `SUNO_API_KEY`/`SUNO_MODEL` in `.env` — unofficial (can break; licensing per Suno's EULA), key stays server-side. App methods: `api.generateSunoSong` / `api.getSunoStatus`.
